    ...
    def validate(self, value: str):
        for val in value.split():
            if not isinstance(val, str):
                raise TypeError(f'Значение {val} должно быть текстом')
            if not val.isalpha():
                raise TypeError(f'Значение {val} должно содержать только буквы')
            if not val.istitle():
                raise TypeError(f'Значение {val} должно начинаться с заглавной буквы')
